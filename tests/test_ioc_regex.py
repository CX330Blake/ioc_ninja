...
def run_tests():
    ...
    add("Email", [
        ...
        ("a@b", False),
        ("toolonglocalpart"*5 + "@example.com", False),
        ("user@-bad.com", False),
        ("user@bad-.com", False),
    ...
    add("SHA256", [
        ...
        ("trail e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855,", True),
        ("nonhex g3b0...", False),
        ("spaces e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855 ", True),
        ("A"*64, True),
        ("A"*63, False),
        ("", False),
    ])
    ...
    add("Registry", [
        ...
        (r"HKEY_LOCAL_MACHINE\\bad|key", False),
        (r"Not a key", False),
    ])
    ...
    # JWT
    add("JWT", [
        ("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0IiwibmFtZSI6IkpvZSIsImlhdCI6MTUxNjIzOTAyMn0.dBjftJeZ4CVP-mB92K27uhbUJU1p1r_wW1gFWFOEjXk", True),
        ("eyJ.x.y", True),
        ("notjwt.aaa", False),
        ("eyJ....", False),
        ("eyJabc.def.ghi", True),
    ...
    for kind, cases in tests.items():
        ...
        if not regex:
            print(f"[WARN] Pattern not found: {kind}")
            continue
        for s, expect in cases:
            total += 1
            ok = bool(regex.search(s))
            if ok != expect:
                failed += 1
